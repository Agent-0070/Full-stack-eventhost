...
export const EventProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const [authStateVersion, setAuthStateVersion] = useState(0); // Force re-renders
  ...
  const updateEvent = async (eventId: string, updates: Partial<Event>): Promise<boolean> => {
    try {
      ...
      setEvents(prev => prev.map(event =>
        event.id === eventId || event._id === eventId ? updatedEvent : event
      ));
      return true;
    } catch (err) {
      handleError(err);
  ...
  const updateEventStatus = async (eventId: string, status: Event['status'], statusData: any): Promise<boolean> => {
    try {
      ...
      // Update the event in the local state with the response from server
      setEvents(prev => {
        const newEvents = prev.map(event => {
          if (event.id === eventId || event._id === eventId) {
            const updatedEventData = {
              ...event,
              ...updatedEvent,
              id: event.id || event._id, // Preserve the ID format
              // Ensure statusDetails is properly merged
              statusDetails: updatedEvent.statusDetails || event.statusDetails
            };
            console.log('📝 Updating event in state:', {
              eventId,
              oldEventDate: event.date,
              newEventDate: updatedEvent.date,
              finalEventDate: updatedEventData.date,
              oldEvent: event,
              updatedEvent,
              finalEvent: updatedEventData
            });
  ...
  const addUserComment = async (userId: string, comment: {
    ...
    try {
      ...
      const resp = await usersAPI.addUserComment(userId, {
