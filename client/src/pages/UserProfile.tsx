...
import { ArrowLeft, Calendar, Globe, Users, Building, Award, Mail, MapPin, Star, MessageSquare } from 'lucide-react';
    ...
    <div className="min-h-screen pt-16 bg-gradient-to-br from-background to-muted/50">
      <div className="container mx-auto px-4 py-8">
        ...
        >
          ...
          {/* User Info Card */}
          <Card className="backdrop-blur-glass bg-gradient-glass border-glass-border/20 shadow-glass mb-8">
            <CardHeader>
              <div className="flex items-start space-x-6">
                <Avatar className="h-24 w-24 border-4 border-glass-border/30">
            ...
            {user.bio && (
              <CardContent>
                <div className="space-y-2">
                  ...
                  <p className={`text-muted-foreground leading-relaxed ${showMoreBio ? '' : 'line-clamp-3'}`}>{user.bio}</p>
                  {user.bio?.length > 200 && (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            {user.companyDescription && (
              <Card className="backdrop-blur-glass bg-gradient-glass border-glass-border/20 shadow-glass">
                <CardHeader>
                ...
                <CardContent>
                  <p className={`text-muted-foreground leading-relaxed ${showMoreCompany ? '' : 'line-clamp-3'}`}>{user.companyDescription}</p>
              ...
              <Card className="backdrop-blur-glass bg-gradient-glass border-glass-border/20 shadow-glass">
                ...
                <CardContent>
                  <div className="flex flex-wrap gap-2">
                    {(showAllCountries ? user.hostingCountries : user.hostingCountries.slice(0, 12)).map((country, index) => (
                      <Badge key={index} variant="secondary">{country}</Badge>
                    ))}
                  </div>
                  {user.hostingCountries.length > 12 && (
                    <button className="text-xs text-primary mt-2" onClick={() => setShowAllCountries(!showAllCountries)}>
              ...
              <Card className="backdrop-blur-glass bg-gradient-glass border-glass-border/20 shadow-glass">
                ...
                <CardContent>
                  <div className="space-y-4">
                    {user.successfulEvents.map((event, index) => (
            ...
            {user?.pastEvents && Array.isArray(user.pastEvents) && user.pastEvents.length > 0 && (
              <Card className="backdrop-blur-glass bg-gradient-glass border-glass-border/20 shadow-glass">
                ...
                <CardContent>
                  <div className="space-y-4">
                    {(showAllPastEvents ? user.pastEvents : user.pastEvents.slice(0, 6)).map((event, index) => (
            ...
            {/* Partners */}
            {Array.isArray(user.partners) && user.partners.length > 0 && (
              <Card className="backdrop-blur-glass bg-gradient-glass border-glass-border/20 shadow-glass lg:col-span-2">
                <CardHeader>
                  <CardTitle className="flex items-center">
                ...
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {(showAllPartners ? user.partners : user.partners.slice(0, 6)).map((partner, index) => (
                      <div key={index} className="p-4 border border-glass-border/30 rounded-lg">
          ...
          {/* Comments Section */}
          <Card className="backdrop-blur-glass bg-gradient-glass border-glass-border/20 shadow-glass mt-8">
                ...
                <div>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {Array.from({ length: 6 }).map((_, i) => (
                      <div key={i} className="p-4 border border-glass-border/30 rounded-lg">
                ...
                <>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {userEvents.slice(0, 6).map((event) => (
                      <div
                        key={event.id}
                        className="p-4 border border-glass-border/30 rounded-lg cursor-pointer hover:bg-glass-light/10 transition-colors"
