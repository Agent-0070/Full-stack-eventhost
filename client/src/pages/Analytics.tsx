...
const Analytics: React.FC = () => {
  ...
  const analyticsData = useMemo(() => {
    ...
    const totalRevenue = safeFilteredEvents.reduce((sum, event) => {
      return sum + (event.ticketCategories?.reduce((catSum, cat) => {
    ...
    const eventPerformance = safeFilteredEvents.map(event => {
      const revenue = event.ticketCategories?.reduce((sum, cat) => {
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <Tabs defaultValue="performance" className="space-y-6">
          <TabsList className="grid w-full grid-cols-4">
            ...
            <TabsTrigger value="distribution">Ticket Distribution</TabsTrigger>
                  ...
                  <ChartContainer config={chartConfig} className="h-[300px]">
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        ...
                        >
                          {analyticsData.ticketDistribution.map((entry, index) => (
