...
import { Calendar, Users, MapPin, Plus, Edit, Trash2, TrendingUp, DollarSign, Receipt, Check, X, Eye } from 'lucide-react';
...
import { paymentReceiptAPI } from '../lib/api';
import axios from 'axios';
import { LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, BarChart, Bar, PieChart, Pie, Cell } from 'recharts';

const Dashboard: React.FC = () => {
  const { events, currentUser, createEvent, updateEvent, deleteEvent } = useEvents();
  ...
  const stats = {
    ...
    totalAttendees: userCreatedEvents.reduce((sum, event) => sum + (Array.isArray(event.booked) ? event.booked.length : event.booked), 0),
    upcomingEvents: userCreatedEvents.filter(event => new Date(event.date) > new Date()).length,
    totalRevenue: userCreatedEvents.reduce((sum, event) => sum + ((Array.isArray(event.booked) ? event.booked.length : event.booked) * (event.price || 0)), 0),
  ...
  const getMonthlyEventData = () => {
    ...
    return months.map((month, index) => {
      ...
      const attendeesInMonth = eventsInMonth.reduce((sum, event) =>
        sum + (Array.isArray(event.booked) ? event.booked.length : event.booked), 0
      );

      const revenueInMonth = eventsInMonth.reduce((sum, event) =>
        sum + ((Array.isArray(event.booked) ? event.booked.length : event.booked) * (event.price || 0)), 0
      );
  ...
  const getTopPerformingEvents = () => {
    return userCreatedEvents
      .map(event => ({
        ...
        attendees: Array.isArray(event.booked) ? event.booked.length : event.booked,
        revenue: (Array.isArray(event.booked) ? event.booked.length : event.booked) * (event.price || 0)
      }))
  ...
  const handleCreateTestTicket = async () => {
    try {
      ...
      const response = await axios.post('/api/tickets/create-test-ticket', {}, {
        headers: {
      ...
      toast({
        ...
        description: `Test ticket ${response.data.ticket.ticketId} has been created successfully.`,
      });
  ...
  const handleCreateSuccess = (eventData: any) => {
    if (editingEvent) {
      ...
      toast({
        title: "Event updated",
        description: `Your event "${eventData.title}" has been successfully updated.`,
      });
    } else {
      createEvent(eventData);
      toast({
        title: "Event created",
        description: `Your event "${eventData.title}" has been successfully created.`,
      });
    }
  ...
  return (
    <div className="min-h-screen pt-16 px-4 md:px-6 lg:px-8">
        ...
        >
          <div className="flex flex-col space-y-4 lg:flex-row lg:items-center lg:justify-between lg:space-y-0">
            ...
            <div className="flex flex-col sm:flex-row gap-2 lg:gap-3">
              <Button
                variant="outline"
                className="backdrop-blur-glass bg-gradient-glass border-glass-border/30 font-semibold"
              ...
              <Dialog open={isCreateModalOpen} onOpenChange={setIsCreateModalOpen}>
                <DialogTrigger asChild>
                  <Button className="bg-gradient-accent border-0 shadow-accent-glow font-semibold">
                    <Plus className="h-4 w-4 mr-2" />
                    Create Event
                  </Button>
                </DialogTrigger>
                <DialogContent className="backdrop-blur-glass bg-gradient-glass border-glass-border/30 shadow-glass max-w-4xl max-h-[90vh] overflow-y-auto">
                  <DialogHeader>
          ...
          <Card className="backdrop-blur-glass bg-gradient-glass border-glass-border/30 shadow-glass">
            <CardContent className="p-3 lg:p-4">
              <div className="flex flex-col lg:flex-row items-center lg:space-x-3 space-y-2 lg:space-y-0">
                <div className="w-10 h-10 rounded-full backdrop-blur-sm bg-glass-light/20 border border-glass-border/30 hover:bg-glass-light/30 transition-colors flex items-center justify-center">
                  <Calendar className="h-4 w-4" />
                </div>
                <div className="text-center lg:text-left">
                  <p className="text-xs lg:text-sm text-muted-foreground font-medium">Created Events</p>
                  <p className="text-xl lg:text-2xl font-bold">{stats.createdEvents}</p>
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        >
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            ...
            <div className="block md:hidden mb-6">
              <div className="backdrop-blur-glass bg-gradient-glass border border-glass-border/30 shadow-glass rounded-lg p-2">
                {/* First row */}
                  ...
                  <Card className="backdrop-blur-glass bg-gradient-glass border-glass-border/30 shadow-glass">
                    <CardContent className="p-6">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="w-12 h-12 rounded-full backdrop-blur-sm bg-glass-light/20 border border-glass-border/30 hover:bg-glass-light/30 transition-colors flex items-center justify-center">
              ...
              <div className="space-y-6">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center space-x-2">
                    <Badge variant="outline" className="bg-glass-light/10 border-glass-border/30">
            ...
            <TabsContent value="payments">
              <div className="space-y-6">
                  ...
                  <CardContent>
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between space-y-4 sm:space-y-0">
                      ...
                      <Button
                        ...
                        className="bg-gradient-accent border-0 shadow-accent-glow font-semibold"
                      >
                        <Plus className="h-4 w-4 mr-2" />
                        Create Test Ticket
                      </Button>
                ...
                <Card className="backdrop-blur-glass bg-gradient-glass border-glass-border/30 shadow-glass">
                  <CardHeader>
                    <CardTitle className="text-lg lg:text-xl font-bold">Recent Payment Receipts</CardTitle>
                  </CardHeader>
                  <CardContent>
                    {loadingReceipts ? (
                      ...
                      <div className="max-h-96 overflow-y-auto scrollbar-thin scrollbar-thumb-glass-border/50 scrollbar-track-transparent space-y-4">
                        {paymentReceipts.map((receipt, index) => (
                          <motion.div
                            ...
                            animate={{ opacity: 1, y: 0 }}
                            transition={{ delay: index * 0.1 }}
                            className="border border-border/50 rounded-lg hover:bg-muted/20 transition-colors backdrop-blur-sm"
                          >
                            {/* Mobile Layout */}
                            <div className="block lg:hidden p-4 space-y-3">
                              {/* Header with name and status */}
                              <div className="flex items-center justify-between">
                                <h4 className="font-bold text-base text-foreground">{receipt.user?.name || 'Unknown User'}</h4>
                                <Badge variant={
                              ...
                              <div className="space-y-1">
                                ...
                                <p className="text-sm font-semibold text-foreground">{receipt.user?.email}</p>
                              </div>

                              {/* Event details */}
                              <div className="space-y-1">
                                <p className="text-sm font-medium text-muted-foreground">Event</p>
                                <p className="text-sm font-semibold text-foreground">{receipt.event?.title}</p>
                              ...
                              <div className="flex justify-between items-center">
                                ...
                                <div className="text-right">
                                  ...
                                  <p className="text-sm font-semibold text-foreground">{receipt.paymentMethod}</p>
                                </div>
                              </div>
                              ...
                              <div className="flex flex-col space-y-2 pt-2">
                                <Button
                                  variant="outline"
                                  size="sm"
                                  onClick={() => window.open(`/api/payment-receipts/${receipt._id}/image`, '_blank')}
                                  className="bg-glass-light/10 border-glass-border/30 hover:bg-glass-light/20 backdrop-blur-sm w-full font-semibold"
                                >
                                ...
                                {receipt.status === 'pending' && (
                                  <div className="flex space-x-2">
                                    <Button
                                      ...
                                      className="bg-glass-light/10 border-glass-border/30 hover:bg-glass-light/20 backdrop-blur-sm hover:text-green-700 border-green-200/50 flex-1 font-semibold"
                                    >
                                    ...
                                    <Button
                                      ...
                                      className="bg-glass-light/10 border-glass-border/30 hover:bg-glass-light/20 backdrop-blur-sm hover:text-red-700 border-red-200/50 flex-1 font-semibold"
                                    >
                                ...
                                {receipt.status === 'confirmed' && (
                                  <Badge variant="default" className="bg-green-100 text-green-800 w-full justify-center py-2 font-bold">
                            ...
                            <div className="hidden lg:flex items-center justify-between p-4">
                              <div className="flex-1">
                                <div className="flex items-center justify-between mb-2">
                                  <span className="font-bold text-lg">{receipt.user?.name || 'Unknown User'}</span>
                                  <Badge variant={
                                    receipt.status === 'confirmed' ? 'default' :
                                    receipt.status === 'rejected' ? 'destructive' : 'secondary'
                                  } className="font-bold">
                              ...
                              <div className="flex space-x-2">
                                <Button
                                  ...
                                  size="sm"
                                  onClick={() => window.open(`/api/payment-receipts/${receipt._id}/image`, '_blank')}
                                  className="bg-glass-light/10 border-glass-border/30 hover:bg-glass-light/20 backdrop-blur-sm font-semibold"
                                >
                                ...
                                {receipt.status === 'pending' && (
                                  <>
                                    <Button
                                      ...
                                      size="sm"
                                      onClick={() => handleConfirmReceipt(receipt._id)}
                                      className="bg-glass-light/10 border-glass-border/30 hover:bg-glass-light/20 backdrop-blur-sm hover:text-green-700 border-green-200/50 font-semibold"
                                    >
                                      <Check className="h-4 w-4 mr-1" />
                                      Confirm
                                    </Button>
                                    <Button
                                      ...
                                      size="sm"
                                      onClick={() => handleRejectReceipt(receipt._id)}
                                      className="bg-glass-light/10 border-glass-border/30 hover:bg-glass-light/20 backdrop-blur-sm hover:text-red-700 border-red-200/50 font-semibold"
                                    >
                                      <X className="h-4 w-4 mr-1" />
                                      Reject
                                    </Button>
                                ...
                                {receipt.status === 'confirmed' && (
                                  <Badge variant="default" className="bg-green-100 text-green-800 font-bold">
                                    <Check className="h-3 w-3 mr-1" />
                                    Confirmed
                                  </Badge>
                                )}
