...
import { ArrowLeft, Calendar, MapPin, Users, Clock, AlertTriangle, CheckCircle, DollarSign, Upload, Image as ImageIcon } from 'lucide-react';
...
const EventDetails: React.FC = () => {
  ...
  const { events, currentUser, isAuthenticated } = useEvents();
                    ...
                    {event.status === 'postponed' && event.statusDetails?.originalDate ? (
                      <>
                        ...
                        <span className="text-xs text-muted-foreground line-through">
                          Originally: {new Date(event.statusDetails.originalDate).toLocaleDateString()} at {event.statusDetails.originalTime}
                        </span>
                      </>
                    ) : (
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        {event.statusDetails.originalDate && (
                          <div className="space-y-1">
                            <p className="text-xs font-medium text-amber-800">Original Date & Time:</p>
                            <p className="text-sm text-amber-700 line-through">
                              {new Date(event.statusDetails.originalDate).toLocaleDateString()} at {event.statusDetails.originalTime}
                            </p>
                          </div>
                        )}
                        <div className="space-y-1">
                          <p className="text-xs font-medium text-amber-800">New Date & Time:</p>
                          <p className="text-sm text-amber-700 font-medium">
                            {new Date(event.date).toLocaleDateString()} at {event.time}
                          </p>
                        </div>

                        {event.statusDetails.originalLocation && event.statusDetails.newLocation &&
                         event.statusDetails.newLocation !== event.statusDetails.originalLocation && (
                          <>
                            <div className="space-y-1">
                              <p className="text-xs font-medium text-amber-800">Original Location:</p>
                              <p className="text-sm text-amber-700 line-through">{event.statusDetails.originalLocation}</p>
                            </div>
                            <div className="space-y-1">
                              <p className="text-xs font-medium text-amber-800">New Location:</p>
